...
export const LogisticsHubView: React.FC<LogisticsHubProps> = ({
  ...
  const [newRepProofPhoto, setNewRepProofPhoto] = useState<string | null>(null);
  ...
  const handleProofPhotoUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        setNewRepProofPhoto(reader.result as string);
        setNewRepPhotoAttached(true);
      };
  ...
  const handleCreateDriverReport = (e: React.FormEvent) => {
    ...
    const newReport = {
      ...
      hasPhoto: Boolean(newRepProofPhoto),
    };
          ...
          <div className="space-y-6 animate-in fade-in duration-200">
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 items-start">
              ...
              <div className="lg:col-span-4 bg-[#0B1222] border border-slate-800/90 rounded-3xl p-4 sm:p-5 shadow-2xl space-y-4">
                ...
                {/* Driver Incident & Parcel Harm Red Report Button */}
                <button
                  type="button"
                  onClick={() => setIsReportModalOpen(true)}
                  className="w-full py-2.5 rounded-xl bg-red-600 hover:bg-red-500 text-white text-xs font-black flex items-center justify-center gap-2 shadow-lg shadow-red-600/30 transition cursor-pointer active:scale-95 border border-red-500 mt-2"
                >
          ...
          <div className="bg-[#0c1322] border border-rose-500/40 rounded-3xl max-w-lg w-full p-5 sm:p-6 shadow-2xl relative my-8 space-y-4 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleCreateDriverReport} className="space-y-4 text-xs">
              ...
              <div>
                ...
                {newRepProofPhoto ? (
                  <div className="relative rounded-xl overflow-hidden border border-rose-500/40 bg-[#10182E] p-2">
                    <img
                      src={newRepProofPhoto}
                      alt="Proof of Harm Preview"
                      className="w-full h-36 object-cover rounded-lg"
                    />
                    <div className="absolute top-3 right-3">
                      <button
                        type="button"
                        onClick={() => setNewRepProofPhoto(null)}
                        className="px-2.5 py-1 rounded-lg bg-red-600 hover:bg-red-500 text-white text-[10px] font-bold shadow-md transition flex items-center gap-1 cursor-pointer"
                      >
                  ...
                  <label
                    htmlFor="proof-harm-photo-upload"
                    className="flex flex-col items-center justify-center gap-1.5 p-4 rounded-xl border-2 border-dashed border-slate-700 hover:border-red-500/60 bg-[#10182E] hover:bg-red-950/10 text-slate-300 transition cursor-pointer group"
                  >
                    <div className="w-9 h-9 rounded-xl bg-red-500/10 text-red-400 flex items-center justify-center group-hover:scale-105 transition">
                      <Camera className="w-4 h-4" />
                    </div>
                    <div className="text-center">
                      <span className="text-xs font-bold text-slate-200 group-hover:text-white block">
                        📸 Click to Upload Photo / Proof of Harm
                      </span>
                      <span className="text-[10px] text-slate-500 block mt-0.5">
                        Parcel damage, road pothole, or accident photo (PNG, JPG)
                      </span>
                    </div>
                    <input
                      id="proof-harm-photo-upload"
                      type="file"
                      accept="image/*"
